...
function MerchantLists() {
  ...
  async function getMerchantList() {
    const merchantRes = await Axios.get(
      "http://localhost:5000/auth/allmerchants"
    );
    setMerchant(merchantRes.data);
  }

  useEffect(() => {
    fetch(`http://localhost:5000/auth/allmerchants?page=${pageNumber}`)
      .then((response) => response.json())
      .then(({ totalPages, merchant }) => {
                    ...
                    >
                      <fieldset>
                        <table className="table">
                          ...
                          { merchant.map((merchant, i) => {
                            return (
                              <tbody>
                                <tr i={i} value={merchant.id}>
                                  <td>{merchant.shopid}</td>
                                  <td>{merchant.shopname}</td>
                                  <td>{merchant.firstname}</td>

                                  <td>{merchant.lastname}</td>
                                  <td>{merchant.email}</td>
                                  <td>{merchant.mobno}</td>
